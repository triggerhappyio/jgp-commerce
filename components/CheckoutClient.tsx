...
export default function CheckoutClient({ checkoutEnabled }: { checkoutEnabled: boolean }) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (data.url) {
        ...
        // Server-side messages are already customer-safe (see
        // app/api/checkout/route.ts) — never a raw Prisma/Stripe error or
        // an env var name, so it's fine to show directly.
    ...
    <main className="section">
      <div className="container" style={{ maxWidth: 640 }}>
        ...
        {checkoutEnabled ? (
          ...
          >
            <div style={{ fontWeight: 600, marginBottom: 4 }}>Online checkout is being activated for launch.</div>
            <p style={{ fontSize: 13, color: "var(--steel)", margin: 0 }}>
              Your cart is saved. In the meantime, visit us in Los Angeles or Buena Park, or reach out
              via the Contact page to place an order.
            </p>
          </div>
        )}
      </div>
