...
export const ENV_CONTRACT: Record<string, { category: EnvCategory; secret: boolean; note: string }> = {
  ...
  STRIPE_SECRET_KEY: { category: "optional", secret: true, note: "Test key in dev/preview, live key only in production. Genuinely optional everywhere — app/checkout/page.tsx shows a polished disabled state (not an error) when unset, so a client demo can run before Stripe is activated." },
  STRIPE_WEBHOOK_SECRET: { category: "optional", secret: true, note: "Per-environment — test and live webhooks have separate secrets. Optional alongside STRIPE_SECRET_KEY; only load-bearing once checkout is actually enabled." },
